...
def _format_parsed_result(parsed: dict, engine: str) -> str:
    """格式化解析结果为可读确认文本"""
    engine_label = {"paddle": "本地 OCR", "deepseek": "AI 识别"}.get(engine, engine)

    lines = [
        f"📋 识别结果（{engine_label}）：\n",
        f"制造商: {parsed.get('manufacturer', '未知')}",
        f"类型: {parsed.get('type', '未知')}",
        f"部位: {parsed.get('slot', '未知')}",
        f"等级: Lv.{parsed.get('level', 0)}",
        f"\n词条:",
    ]

    for i, affix in enumerate(parsed.get("affixes", []), 1):
